...
def download_twitch_video(video_url, output_file):
    ...
    command = ["TwitchDownloaderCLI/TwitchDownloaderCLI", "videodownload", "--id", video_id, "-o", output_file, "--threads", "20", "--temp-path", "temp"]
...
def upload_video_to_vk(token, group_id, video_path, album_id, name, description, privacy_view="all"):
    ...
    # Потоковая загрузка с использованием MultipartEncoder
