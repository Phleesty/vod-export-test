...
def download_twitch_video(video_url, output_file, progress_dict, lock, thread_id):
    ...
    downloading_end_time = None
    is_downloading = False
    
    while process.poll() is None:
        line = process.stdout.readline().strip()
        if line and "may not have enough free space" not in line:
            if "Downloading" in line:
                if not is_downloading:
                    downloading_start_time = datetime.now()
                    is_downloading = True
                with lock:
                    progress_dict[thread_id] = f"[{video_id}] {line}"
            elif "Fetching Video Info" in line or "Verifying Parts" in line or "Finalizing Video" in line:
                if is_downloading:
                    downloading_end_time = datetime.now()
                    is_downloading = False
                with lock:
                    progress_dict[thread_id] = f"[{video_id}] {line}"
    ...
    if downloading_start_time and downloading_end_time:
        download_time = (downloading_end_time - downloading_start_time).total_seconds()
    else:
        download_time = 0
    ...
    msg = (f"Файл {output_file} ({file_size:.2f} МБ) скачан за "
           ...
           f"Файл полностью обработан за {int(total_time // 60)} мин {int(total_time % 60)} сек.")
    with lock:
        progress_dict[thread_id] = msg
    logging.info(msg)

def display_progress(progress_dict, lock, stop_event):
    """Отображает прогресс для всех потоков в консоли без очистки истории."""
    num_threads = len(progress_dict)
    while not stop_event.is_set():
        with lock:
            # Перемещаем курсор к началу блока прогресса
            sys.stdout.write(f"\033[{num_threads + 1}A")
...
def main(start_row=1, end_row=None, do_vk_upload=True, do_odysee_upload=True, debug=False):
    ...
    for index in range(start_index, end_index):
        ...
        # Выводим начальные строки для прогресса
        for i in range(len(video_urls)):
            print(f"[{video_urls[i].split('/')[-1]}] ", end="")
