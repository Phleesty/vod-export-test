...
# Функция для скачивания видео с Twitch (в потоке)
...
def add_part_to_title(title, part_number):
    # Найти позицию последней открывающей скобки
    last_open_paren = title.rfind("(")
    if last_open_paren == -1:
        # Если нет скобок, добавляем "Часть N" в конец с точкой
        return f"{title}. Часть {part_number}"
    
    # Извлечь дату (часть в скобках)
    date_part = title[last_open_paren:]
    # Извлечь основное название до даты
    main_title = title[:last_open_paren].strip()
    
    # Проверить, есть ли знак препинания в конце основного названия
    if main_title and main_title[-1] in ".!?":
        # Если есть, добавляем " Часть N" без точки
        new_main_title = f"{main_title} Часть {part_number}"
    else:
        # Если нет, добавляем ". Часть N"
        new_main_title = f"{main_title}. Часть {part_number}"
    
    # Соединяем основное название и дату
    ...
    for index in range(start_index, end_index):
        ...
        for url in video_urls:
            ...
            if not os.path.exists(output_file):
                thread = threading.Thread(target=download_twitch_video, args=(url, output_file))
